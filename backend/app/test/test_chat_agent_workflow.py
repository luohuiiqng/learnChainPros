...
class WorkflowPlanner(BasePlanner):
    def plan(self, input_data: Any, context: Any = None) -> dict[str, Any]:
        return {
            "action": "workflow",
            "reason": "workflow planner direct workflow branch",
            "tool_name": None,
...
def test_chat_agent_workflow_planner_time_then_model() -> None:
    ...
    agent_input = AgentInput(message="现在几点了？", session_id="pytest-workflow-session")
    ...
    assert (
        runtime_session.planner_result["reason"]
        == "workflow planner direct workflow branch"
    )
    ...
    assert runtime_session.workflow_trace[0]["input_summary"] == "现在几点了？"
    assert (
        runtime_session.workflow_trace[0]["output_summary"]
        == runtime_session.planner_result["reason"]
    )
    ...
    assert (
        runtime_session.workflow_trace[1]["output"]
        == runtime_session.workflow_result["results"][0]["output"]
    )
    assert (
        runtime_session.workflow_trace[1]["input_summary"]
        == "tool=time_tool,params={'content': '现在几点了？'}"
    )
    assert runtime_session.workflow_trace[1]["output_summary"] == (
        f"tool returned: {runtime_session.workflow_result['results'][0]['output']}"
    )
    ...
    assert (
        runtime_session.workflow_trace[2]["output"]
        == runtime_session.workflow_result["results"][1]["output"]
    )
    assert "当前时间是 " in runtime_session.workflow_trace[2]["input_summary"]
    assert runtime_session.workflow_trace[2]["output_summary"] == (
        f"model returned: {runtime_session.workflow_result['results'][1]['output']}"
    )
    ...
    assert (
        runtime_session.tool_calls[0]["output"]
        == runtime_session.workflow_result["results"][0]["output"]
    )
    ...
    assert (
        runtime_session.workflow_result["results"][0]["output"]
        in runtime_session.model_calls[0]["prompt"]
    )
    assert runtime_session.model_calls[0]["success"] is True
    assert (
        runtime_session.model_calls[0]["output"]
        == runtime_session.workflow_result["results"][1]["output"]
    )
    ...
    assert (
        runtime_session.workflow_result["results"][0]["output"]
        in mock_status["last_input"].prompt
    )
